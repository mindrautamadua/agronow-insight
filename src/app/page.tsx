...
import {
  GraduationCap, BookOpen, Clock, Receipt, Users, Wallet, Coins,
  Building2, ChevronDown, Sparkles, Layers, BarChart3,
  Search, CheckCircle2, Target, AlertTriangle, X, ChevronRight, ChevronLeft,
} from "lucide-react";
import { fetchDashboard, fetchCapaian, fetchBiaya, fetchDaftar, type DashboardData, type DashMonth, type DashSubkelompok, type CapaianData, type JplEmployee, type BiayaData, type BiayaBar, type DaftarData, type DaftarTraining } from "@/lib/data";
...
const TABS = ["Dashboard", "Capaian JPL", "Biaya"] as const;
type Tab = (typeof TABS)[number];

// Jenis detail (list) yang muncul saat sebuah KPI card di Dashboard diklik.
type KpiKind = "sesi" | "jpl" | "biaya" | "peserta" | "biayaPerKaryawan" | "jplPerKaryawan";
...
const METRICS: { key: Metric; label: string }[] = [
  { key: "jpl", label: "JPL" }, { key: "sesi", label: "Program" },
...
export default function DashboardPage() {
  ...
  const kpis: { label: string; value: string; sub: string; icon: React.ElementType; tone: string; kind: KpiKind }[] = [
    { label: "Program", value: fmtNum(kpi.sesi), sub: "kelas terlaksana", icon: BookOpen, tone: "text-violet-500 bg-violet-500/10 border-violet-500/20", kind: "sesi" },
    ...
    { label: "Peserta", value: fmtNum(kpi.peserta), sub: "karyawan terlibat", icon: Users, tone: "text-blue-500 bg-blue-500/10 border-blue-500/20", kind: "peserta" },
    { label: "Rata-rata Biaya/Karyawan", value: fmtRupiah(kpi.peserta ? kpi.biaya / kpi.peserta : 0), sub: "biaya per peserta", icon: Coins, tone: "text-teal-500 bg-teal-500/10 border-teal-500/20", kind: "biayaPerKaryawan" },
    { label: "Rata-rata JPL/Karyawan", value: (kpi.peserta ? Math.round((kpi.jpl / kpi.peserta) * 10) / 10 : 0).toLocaleString("id-ID"), sub: "JPL per peserta", icon: GraduationCap, tone: "text-rose-500 bg-rose-500/10 border-rose-500/20", kind: "jplPerKaryawan" },
  ];
    ...
    <div className="p-6 max-w-[1400px] mx-auto space-y-5">
      ...
      {tab === "Capaian JPL" ? (
        ...
        <div className="space-y-5">
          ...
          <SubkelompokCard rows={data.perSubkelompok} onPick={key => setDetailReq({ mode: "subkelompok", key })} />
...
function buildInsights(monthly: DashMonth[]): string[] {
  ...
  out.push(`Terbanyak kelas di ${BULAN[mostClass.bln - 1]} — ${mostClass.sesi} program`);
...
// ── Realisasi JPL per subkelompok 70-20-10 ─────────────────────────────────────
// Bandingkan komposisi JPL aktual (formal/sosial/pengalaman) dengan porsi ideal
// kerangka 70-20-10. Garis putus-putus pada bar = target ideal tiap kelompok.
const SUB_INFO: Record<string, { tier: string; nama: string; desc: string; color: string }> = {
  metode_belajar70: { tier: "70", nama: "Pengalaman Kerja", desc: "Project assignment · action learning", color: "#10b981" },
  metode_belajar20: { tier: "20", nama: "Sosial & Coaching", desc: "Coaching · mentoring · benchmark", color: "#3b82f6" },
  metode_belajar10: { tier: "10", nama: "Formal / Kelas", desc: "Workshop · kelas · belajar mandiri", color: "#f59e0b" },
};

function SubkelompokCard({ rows, onPick }: { rows: DashSubkelompok[]; onPick?: (key: string) => void }) {
  const total = rows.reduce((s, r) => s + r.jpl, 0);
  ...
  return (
    <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] p-5">
      <div className="flex items-start justify-between gap-3 flex-wrap mb-1">
        <h3 className="text-sm font-bold text-[var(--foreground)] flex items-center gap-2">
          <Sparkles className="w-4 h-4 text-emerald-500" /> Realisasi JPL · Model 70-20-10
          <span className="text-[var(--muted)] font-normal text-xs">· klik untuk rincian pelatihan</span>
        </h3>
        <span className="text-[11px] text-[var(--muted)]">total {fmtNum(total)} JPL</span>
      </div>
      <p className="text-[11px] text-[var(--muted)] mb-4">
        Komposisi JPL aktual per metode belajar dibanding porsi ideal · garis putus-putus = target
      </p>
      ...
      {total === 0 ? (
        ...
        <div className="space-y-1">
          {rows.map(r => {
            const info = SUB_INFO[r.key] ?? { tier: "?", nama: r.key, desc: "", color: "#94a3b8" };
            const actual = pct(r.jpl);
            const gap = actual - r.ideal;
            const clickable = !!onPick && r.jpl > 0;
            const inner = (
              <>
                <span className="w-9 h-9 shrink-0 rounded-lg flex items-center justify-center text-sm font-bold tabular-nums border"
                  style={{ color: info.color, background: `${info.color}1a`, borderColor: `${info.color}33` }}>
                  {info.tier}
                </span>
                <div className="min-w-0 w-[150px] shrink-0 text-left">
                  <p className="text-xs font-semibold text-[var(--foreground)] truncate" title={info.nama}>{info.nama}</p>
                  <p className="text-[10px] text-[var(--muted)] truncate" title={info.desc}>{info.desc}</p>
                </div>
                <div className="flex-1 min-w-[80px]">
                  {/* track 0–100% · fill = aktual · tick = ideal */}
                  <div className="relative h-2.5 rounded-full bg-[var(--bg-card2)] overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: `${actual}%`, background: info.color }} />
                  </div>
                  <div className="relative h-0">
                    <span className="absolute -top-2.5 w-px h-2.5 bg-[var(--foreground)]/50"
                      style={{ left: `${r.ideal}%` }} title={`ideal ${r.ideal}%`} />
                  </div>
                </div>
                <div className="shrink-0 text-right w-[92px]">
                  <p className="text-sm tabular-nums">
                    <span className="font-bold text-[var(--foreground)]">{actual}%</span>
                    <span className="text-[10px] text-[var(--muted)]"> / {r.ideal}%</span>
                  </p>
                  <p className="text-[10px] tabular-nums" style={{ color: Math.abs(gap) <= 5 ? "var(--muted)" : gap > 0 ? "#f59e0b" : "#3b82f6" }}>
                    {gap > 0 ? "+" : ""}{gap} pp · {fmtNum(r.jpl)} JPL
                  </p>
                </div>
              </>
            );
            return clickable ? (
              <button key={r.key} type="button" onClick={() => onPick!(r.key)}
                className="group w-full flex items-center gap-3 rounded-lg -mx-1.5 px-1.5 py-1.5 transition-colors hover:bg-[var(--bg-card2)]/60">
                {inner}
                <ChevronRight className="w-4 h-4 shrink-0 -ml-1 text-[var(--muted)] opacity-0 group-hover:opacity-100 transition-opacity" />
              </button>
            ) : (
              <div key={r.key} className="flex items-center gap-3 px-1.5 py-1.5">{inner}</div>
            );
          })}
        </div>
      )}
  ...
  return (
    <li>
      <button type="button" onClick={() => onPick?.(e.id, e.nama)} disabled={!onPick}
        ...
        <span className="shrink-0 w-12 text-right text-[11px] text-[var(--muted)] tabular-nums">{e.sesi} program</span>
...
function BiayaView({ data, loading, onPick }: { data: BiayaData | null; loading: boolean; onPick?: (by: "unit" | "penyelenggara" | "level" | "kategori", label: string) => void }) {
  ...
  const cards = [
    ...
    { label: "Program berbiaya", value: fmtNum(kpi.sesiBerbiaya), sub: `${fmtNum(kpi.sesiTanpaBiaya)} tanpa biaya`, icon: BookOpen, tone: "text-violet-500 bg-violet-500/10 border-violet-500/20" },
  ];
  return (
    <div className={`space-y-5 transition-opacity ${loading ? "opacity-60" : ""}`}>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
...
// ── Helper format (dipakai modal detail) ──────────────────────────────────────
...
const KIND_META: Record<KpiKind, { title: string; icon: React.ElementType }> = {
  sesi:        { title: "Daftar Program / Kelas", icon: BookOpen },
  jpl:         { title: "JPL per Pelatihan", icon: Clock },
  biaya:       { title: "Biaya per Pelatihan", icon: Receipt },
  peserta:     { title: "Peserta (karyawan terlibat)", icon: Users },
  biayaPerKaryawan: { title: "Biaya per Karyawan", icon: Coins },
  jplPerKaryawan: { title: "JPL per Karyawan", icon: GraduationCap },
};
...
function DetailModal({ req, data, loading, onClose }: { req: Detail; data: DaftarData | null; loading: boolean; onClose: () => void }) {
  ...
  const isCostPerEmp = req.mode === "kpi" && req.kind === "biayaPerKaryawan";
  const isJplPerEmp = req.mode === "kpi" && req.kind === "jplPerKaryawan";
  const byBiaya = showBiaya || isCostPerEmp; // tampilkan & urutkan peserta berdasarkan biaya
  const isPeserta = req.mode === "level" || (req.mode === "kpi" && req.kind === "peserta")
    || isCostPerEmp || isJplPerEmp
  ...
  const { title, Icon } = useMemo(() => {
    ...
    if (req.mode === "subkelompok") { const info = SUB_INFO[req.key]; return { title: `Pelatihan · ${info ? `${info.tier} ${info.nama}` : req.key}`, Icon: Sparkles }; }
  ...
  const pesertaAgg = useMemo(() => {
    ...
    return [...m.values()].sort((a, b) => byBiaya ? (b.biaya - a.biaya || a.nama.localeCompare(b.nama)) : (b.jpl - a.jpl || a.nama.localeCompare(b.nama)));
  }, [isPeserta, data, req, byBiaya]);
  ...
  const trainings = useMemo(() => {
    ...
    else if (req.mode === "subkelompok") arr = arr.filter(t => t.sub === req.key);
    ...
    else arr.sort((a, b) => b.jpl - a.jpl); // jpl
    return arr;
  ...
  const summary = (() => {
    if (isPeserta) {
      if (byBiaya) {
        const totBiaya = fPeserta.reduce((a, p) => a + p.biaya, 0);
        return isCostPerEmp
          ? `${fmtNum(fPeserta.length)} karyawan · rata-rata ${fPeserta.length ? rupiahFull(totBiaya / fPeserta.length) : "—"}/karyawan`
          : `${fmtNum(fPeserta.length)} peserta · total ${rupiahFull(totBiaya)}`;
      }
      const totJpl = fPeserta.reduce((a, p) => a + p.jpl, 0);
      return isJplPerEmp
        ? `${fmtNum(fPeserta.length)} karyawan · rata-rata ${fPeserta.length ? (Math.round((totJpl / fPeserta.length) * 10) / 10).toLocaleString("id-ID") : "—"} JPL/karyawan`
        : `${fmtNum(fPeserta.length)} peserta · total ${fmtNum(totJpl)} JPL`;
    }
    if (valueKind === "biaya") return `${fTrain.length} pelatihan · total ${rupiahFull(fTrain.reduce((a, t) => a + t.biaya, 0))}`;
    if (valueKind === "sesi") return `${fTrain.length} program · ${fmtNum(fTrain.reduce((a, t) => a + t.peserta, 0))} kehadiran`;
                        ...
                        <div className="shrink-0 text-right">
                          {byBiaya ? (
                            <>
                              ...
                              <p className="text-[10px] text-[var(--muted)] tabular-nums">{p.sesi} program · {fmtNum(p.jpl)} JPL</p>
                            </>
                          ) : (
                            <>
                              ...
                              <p className="text-[10px] text-[var(--muted)] tabular-nums">{p.sesi} program</p>
                            </>
                          )}
                        </div>
                        <ChevronRight className="w-4 h-4 shrink-0 text-[var(--muted)] opacity-0 group-hover:opacity-100 transition-opacity" />
  ...
  return (
    <>
      ...
      <div className="px-5 pt-4 grid grid-cols-3 gap-2">
        ...
        <Stat label="Program" value={fmtNum(rows.length)} />
